...
if(0 == $id_testa) {
	...
	print("<div style='text-align:center;'><b>PL / PB n: ".$plInfo->getField(numerodoc)."</b></div></br>");
    ...
    while(!$pb->EOF) {
    	...
    	if ((int)$pb->getField(quantita)==0){			//RIGHE DI DESCRIZIONE PACKINGLIST
			...
			if ((int)$bancale != (int)$pb->getField(bancale)) {  //strnatcmp($bancale, $pb->getField(bancale)!=0)){
				...
				if($bancale == 0){
					...
					if($bancale != 0 && $pb->getField(collo) == 0){
						$pesoBanc=$pb->getField(peso)*$pb->getField(quantita);
			...
			else
			{
				if($pb->getField(imballo)){
					...
					celleVuote(1);
					print("<td><a href='pl-banc.php?num=".$plInfo->getField(numerodoc)."&anno=$anno&btnok=Colli&id_testa=0' target='_blank'><img noborder src='../img/modify.png' height=32></a></td>\n");
					if($pb->getField(quantitare)>0) print("<td><a href='pl-delrow.php?id=".$pb->getField(id_riga)."&type=IMB'><img noborder src='../img/delete.png' height=32></a></td>\n");
					$pesoCollo=0;
				} else {
					$pesoCollo=$pesoCollo+$pb->getField(peso)*$pb->getField(quantita);
	...
	print("<br><br>");
	print("<a href=\"reprint-pl-select.php?num=".$plInfo->getField(numerodoc)."&anno=$anno\" target=\"_blank\"><button>Ristampa Colli</button></a>  &nbsp;&nbsp;");
	print("<a href=\"reprint-banc-select.php?num=".$plInfo->getField(numerodoc)."&anno=$anno\" target=\"_blank\"><button>Ristampa Bancali</button></a>");
	//print("<button>Cambia Data Evasione</button>");
	print("</div></br>");
