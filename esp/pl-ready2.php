...
print("<h3 class='title'>Tutte le PL in Partenza</h3>");
/*
print("<div style='text-align:center;'>");
print("<p style='font-weight:bold;'>Data di Partenza:</p>");
print("<form action='pl-ready2.php'>");
...
print("<ul>");
if ($codNation==0) {
	print("<li><a class='selected' href='pl-ready2.php?nat=0&data=$data'>FRANCE</a></li>");
	print("<li><a href='pl-ready2.php?nat=1&data=$data'>ESTERO</a></li>");
} else {
	print("<li><a href='pl-ready2.php?nat=0&data=$data'>FRANCE</a></li>");
	print("<li><a class='selected' href='pl-ready2.php?nat=1&data=$data'>ESTERO</a></li>");
}
...
//CERCO I VETTORI
if($codNation==0){
	$Query = "SELECT DISTINCT doctes.vettore1 as codvettore, vettori.descrizion as vettore FROM doctes LEFT OUTER JOIN anagrafe on doctes.codicecf==anagrafe.codice LEFT OUTER JOIN vettori on doctes.vettore1==vettori.codice WHERE doctes.tipodoc=='PL' AND doctes.nomodifica==.F. AND anagrafe.codnazione=='F' AND doctes.esercizio='2015'";
} else {
	$Query = "SELECT DISTINCT doctes.vettore1 as codvettore, vettori.descrizion as vettore FROM doctes LEFT OUTER JOIN anagrafe on doctes.codicecf==anagrafe.codice LEFT OUTER JOIN vettori on doctes.vettore1==vettori.codice WHERE doctes.tipodoc=='PL' AND doctes.nomodifica==.F. AND anagrafe.codnazione<>'F' AND doctes.esercizio='2015'";
}
...
while(!$dbVettori->EOF){
	...
	if($codVett == $vett){
		print("<li><a class='selected' href='pl-ready2.php?vett=".$vett."&nat=$codNation&data=$data''>".$dbVettori->getField(vettore)."</a></li>");
	} else {
		print("<li><a href='pl-ready2.php?vett=".$vett."&nat=$codNation&data=$data''>".$dbVettori->getField(vettore)."</a></li>");
	}
	$dbVettori->moveNext();
}
print("</ul></div>");
...
//INFORMAZIONI TESTA
if($codNation==0){
	$Query = "SELECT doctes.id, doctes.numerodoc, doctes.codicecf, doctes.datadoc, doctes.nomodifica, vettori.descrizion as vettore, anagrafe.descrizion, nazioni.descrizion as Nazione FROM doctes LEFT OUTER JOIN anagrafe on doctes.codicecf==anagrafe.codice LEFT OUTER JOIN nazioni on nazioni.codice == anagrafe.codnazione LEFT OUTER JOIN vettori on doctes.vettore1==vettori.codice WHERE doctes.tipodoc=='PL' AND doctes.nomodifica==.F. AND anagrafe.codnazione=='F' AND doctes.vettore1=='$codVett' AND doctes.esercizio='2015' ORDER BY doctes.datadoc";
} else{
	$Query = "SELECT doctes.id, doctes.numerodoc, doctes.codicecf, doctes.datadoc, doctes.nomodifica, vettori.descrizion as vettore, anagrafe.descrizion, nazioni.descrizion as Nazione FROM doctes LEFT OUTER JOIN anagrafe on doctes.codicecf==anagrafe.codice LEFT OUTER JOIN nazioni on nazioni.codice == anagrafe.codnazione LEFT OUTER JOIN vettori on doctes.vettore1==vettori.codice WHERE doctes.tipodoc=='PL' AND doctes.nomodifica==.F. AND anagrafe.codnazione<>'F' AND doctes.vettore1=='$codVett' AND doctes.esercizio='2015' ORDER BY doctes.datadoc";
	// doctes.u_note,
}
...
print("<thead>\n");
	print("<tr>");
	    ...
	    print("<th style='width: 100px;'>Data</th>");
	    print("<th style='width: 40px;'>Evasa</th>");
...
while(!$db->EOF) {
	...
	print("<td style='text-align: center;'>".date("d/m/Y", strtotime($db->getField(datadoc)))."</td>");
	//GESTIONE SEMAFORO
	if ($db->getField(nomodifica)){
		print("<td style='text-align: center;'><img noborder src='../img/semaforoVerde.png' height=32></td>");
	} else {
		print("<td style='text-align: center;'><img noborder src='../img/semaforoRosso.png' height=32></td>");
	}
