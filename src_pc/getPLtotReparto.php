...
if($gestPB){
	$Query = "SELECT COUNT(DISTINCT docrig.numeroriga) as nonprep from docrig where docrig.codicearti<>'' and docrig.quantitare>0 AND docrig.id_testa==$cId";
} else {
...
if($gestPB){
	$Query = "SELECT COUNT(DISTINCT docrig.u_costk) as prep from docrig where docrig.u_costk<>0 AND docrig.u_costk1==0 AND docrig.riffromt==$cId";
} else {
...
//CONTO I BANCALATI
if($gestPB){
	$Query = "SELECT COUNT(DISTINCT docrig.u_costk1) as banc from docrig where docrig.u_costk1<>0 AND docrig.riffromt==$cId";
} else {
