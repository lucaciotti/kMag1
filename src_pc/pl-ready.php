...
print("<thead>\n");
	print("<tr>");
	    ...
	    print("<th style='width: 40px;'>Data/Vett.</th>");
...
while(!$db->EOF) {
	...
	//GESTIONE DATA VETTORE
	if ($db->getField('nomodifica') && 1>2){
		print("<td style='text-align: center;'></td>");
	} else {
		if ($db->getField('numrighepr')==0){
			print("<td style='text-align: center;'><a href='pl-changeDate.php?id_testa=".$db->getField('id')."' target=\"_blank\"><button><img noborder src='../img/modify.png' height=24></button></a></td>");
		} else {
			print("<td style='text-align: center;'><a href='pl-changeDate.php?id_testa=".$db->getField('id')."' target=\"_blank\"><button><img noborder src='../img/modify.png' height=24></button></a></td>");
		}
	}	
